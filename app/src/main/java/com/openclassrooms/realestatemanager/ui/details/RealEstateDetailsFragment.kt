...
class RealEstateDetailsFragment : Fragment(), OnMapReadyCallback {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (id == null || id == nullValue) {
            ...
            binding.emptyDetailsView.title.text = getString(R.string.no_details_available_text)
            binding.emptyDetailsView.subTitle.text =
                getString(R.string.no_details_available_subtext)
        } else {
                ...
                viewModel.detailsUiState.collect { uiState ->
                    when (uiState) {
                        ...
                        is DetailsViewModel.DetailsUiState.Error -> {
                            ...
                            binding.emptyDetailsView.title.text = getString(R.string.no_details_available_text)
                            binding.emptyDetailsView.subTitle.text =
                                getString(R.string.no_details_error)
                        }
    ...
    private fun setupView(realEstate: RealEstateData) {
        ...
        val utilsKt = UtilsKt()
        binding.priceTextView.text = utilsKt.formatCurrency(realEstate.price, true)
        ...
        binding.currencySwitch.setOnClickListener {
            if (isChecked) {
                ...
                binding.priceTextView.text =
                    utilsKt.formatCurrency(realEstate.price, false)
                isChecked = true
            }
        }
        ...
        if (realEstate.photos.isNotEmpty()) {
            ...
            binding.noPicTextView.text = getString(R.string.no_pictures_error_details)
        }

        if (realEstate.isAvailable) {
            binding.availabilityTextView.text = getString(R.string.item_available)
        } else {
            binding.availabilityTextView.text = getString(R.string.item_sold_capitalized)
        }
        ...
        if (realEstate.lat == 0.0 && realEstate.lon == 0.0){
            ...
            binding.emptyMapView.title.text = getString(R.string.no_location_details_title)
            binding.emptyMapView.subTitle.text = getString(R.string.no_location_details)
        } else {
